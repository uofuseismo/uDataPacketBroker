...
#include "uDataPacketBrokerAPI/v1/publish_service.grpc.pb.h"
#include "uDataPacketBrokerAPI/v1/packet.pb.h"
#include "uDataPacketBrokerAPI/v1/publish_response.pb.h"
...
class AsynchronousReader :
    public grpc::ServerReadReactor<UDataPacketBrokerAPI::V1::Packet>
{
public:
    using Packet = UDataPacketBrokerAPI::V1::Packet;
    using PublishResponse = UDataPacketBrokerAPI::V1::PublishResponse;
    ...
    void OnCancel() override
    {
        ...
        {
            SPDLOG_LOGGER_INFO(mLogger,
                "Async packet broker publish service RPC canceled by {}",
                mPeer);
        }
        else
        {
    ...
    ~AsynchronousReader()
    {
        if (mLogger)
        {
            SPDLOG_LOGGER_DEBUG(mLogger, "In async packet reader destructor");
        }
    }   
#endif
...
//private:
    ...
    UDataPacketBrokerAPI::V1::PublishResponse *mResponse{nullptr};
    std::function<void (UDataPacketBrokerAPI::V1::Packet &&)> mCallback;
    const std::atomic<bool> *mKeepRunning{nullptr};
    std::atomic<int> *mPublisherCount{nullptr};
    std::shared_ptr<spdlog::logger> mLogger;
    UDataPacketBrokerAPI::V1::Packet mCurrentPacket;
...
class PublishService::PublishServiceImpl :
    public UDataPacketBrokerAPI::V1::PublishService::CallbackService
{
public:
    PublishServiceImpl
    (
        ...
        mLogger(std::move(logger))
    {
        ...
        if (mLogger == nullptr)
        {
            ...
            constexpr const char *loggerName{"PublishServiceConsole"};
    ...
    grpc::ServerReadReactor<UDataPacketBrokerAPI::V1::Packet>*
        Publish(grpc::CallbackServerContext* context,
                UDataPacketBrokerAPI::V1::PublishResponse *publishResponse) override
    {
